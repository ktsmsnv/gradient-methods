...
void findAB(double x1, double x2, double z1, double z2, int h, double& a, double& b, int& N0)
{
	double y1, y2;
	int i;
	y1 = 104 * pow(x1, 2) - 206 * x1 * x2 + 104 * pow(x2, 2) - 100 * x1 + 8 * x2 + 100;
	i = 1;
	while (1)
	{
		y2 = 104 * pow((x1 - (i - h) * z1), 2) - 206 * (x1 - (i - h) * z1) * (x2 - (i - h) * z2) + 104 * pow((x2 - (i - h) * z2), 2) - 100 * (x1 - (i - h)*z1) + 8 * (x2 - (i - h) * z2) + 100;
		N0 = N0 + 1;
		if (y1 > y2)
		{
			i = i + 1;
...
void find(double a, double b, double x1, double x2, double z1, double z2, double& lam, int& N0)
{
	...
	lam0 = b - (b - a) / t;
	lam1 = a + (b - a) / t;
	y0 = 104 * pow((x1 - lam0 * z1), 2) - 206 * (x1 - lam0 * z1) * (x2 - lam0 * z2) + 104 * pow((x2 - lam0 * z2), 2) - 100 * (x1 -
		lam0 * z1) + 8 * (x2 - lam0 * z2) + 100;
	y1 = 104 * pow((x1 - lam1 * z1), 2) - 206 * (x1 - lam1 * z1) * (x2 - lam1 * z2) + 104 * pow((x2 - lam1 * z2), 2) - 100 * (x1 -
		lam1 * z1) + 8 * (x2 - lam1 * z2) + 100;
	N0 = 2;
	do
	{
		if (y0 < y1)
	...
	lam = (a + b) / 2;
	egar = (b - a) / 2;
	y = 104 * pow((x1 - lam * z1), 2) - 206 * (x1 - lam * z1) * (x2 - lam * z2) + 104 * pow((x2 - lam * z2), 2) - 100* (x1 - lam * z1) + 8 * (x2 -
		lam * z2) + 100;
}
void spusk(double a, double b)
{
	...
	do
	{
		...
		x1l = x1 - lam * z1;
		x2l = x2 - lam * z2;
		z1 = 208 * x1l - 206 * x2l - 100;
		z2 = -206 * x1l + 208 * x2l + 8;
		N1 = N1 + 2;
		l = l + 1;
		x1 = x1l;
		x2 = x2l;
	...
	xk = x1;
	yk = x2;
	y = 104 * pow(xk, 2) - 206 * xk * yk + 104 * pow(yk, 2) - 100 * xk + 8 * yk - 100;
	N = N0 + N1;
	egar = sqrt((pow((x1 - 23.13043), 2)) + (pow((x2 - 22.86957), 2)));
...
void droblshag(double a, double b)
{
	...
	do
	{
		y = 104 * pow(x1, 2) - 206 * x1 * x2 + 104 * pow(x2, 2) - 100 * x1 + 8 * x2 + 100;
		N0 = N0 + 1;
		do
		{
			y1 = 104 * pow((x1 - lam * z1), 2) - 206 * (x1 - lam * z1) * (x2 - lam * z2) + 104 * pow((x2 -
				lam * z2), 2) - 100 * (x1 - lam * z1) + 8 * (x2 - lam * z2) + 100;
			N0 = N0 + 1;
			lam = lam / 2;
		} while (y <= y1);
		lam = lam * 2;
